...
# Com o grep abaixo, direcionaremos os nomes das chamadas que têm erros, ou seja, as chamadas cujo tipo é um número negativo
# Para tal, usamos o grep para retornar somente as linhas que têm "-"
# Porém, sendo "-" um caracter especial, usado para a especificações de opções de um comando, usamos a barra invertida (\)
# para que "-" se comporte como uma string, não como caracter especial
# Como queremos somente o nome, usamos o cut para pegar tudo do primeiro campo delimitado po "(" (já que o nome é seguido por parâmetros
# em parênteses)
...
# O que está sendo feito em seguida é basicamente um algoritmo de checagem de maior ocorrência de erros por chamada
# Essa checagem é feita por meio de um for, que vai checar todos os nomes e a frequência destes por meio do grep
# e é feita a comparação com a frequência de nomes anteriores; se a frequência for maior, há a substituição do valor da variável erros
